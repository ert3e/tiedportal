...
class Autolink {
    ...
    static function autolink($text, $limit=30, $tagfill='', $auto_title = true){
    ...
    ####################################################################
    static function autolink_do($text, $sub, $limit, $tagfill, $auto_title, $force_prefix=null){
        ...
        while (($cursor < strlen($text)) && $loop){
            ...
            if ($ok){
                if (preg_match('/^([a-z0-9\-\.\/\-_%~!?=,:;&+*#@\(\)\$]+)/i', $post, $matches)){
                    ...
                    $buffer .= "<a target=\"_blank\" href=\"{$link_url_enc}\"$currentTagfill>{$display_url_enc}</a>";
    ...
    static function autolink_label($text, $limit){
    ...
    static function autolink_email($text, $tagfill=''){
